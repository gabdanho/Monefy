        ...
        Box(
            ...
            Canvas(
                ...
                for (i in categoriesToSumFinance.keys.indices) {
                    // Тень категории
                    drawArc(
                        color = Color(categoriesToSumFinance.keys.toList()[i].colorLong).darken(0.4f),
                    ...
                    if (finances.isNotEmpty()) {
                        ...
                        scale(scale) {
                            drawArc(
                                color = Color(categoriesToSumFinance.keys.toList()[i].colorLong),
                                startAngle = startAngle,
                                sweepAngle = sweepAnglePercentage[i],
                                useCenter = false,
                                style = Stroke(
        ...
        Row(
            ...
            Row(
                ...
                Canvas(Modifier.size(10.dp)) {
                    drawCircle(
                        color = Color(category.colorLong)
                    )
                }
                Text(
                    text = category.name,
